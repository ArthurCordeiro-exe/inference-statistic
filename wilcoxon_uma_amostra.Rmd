...
# wilcox.test(
#   x,
#       Vetor de dados (a amostra observada).
#
#   y = NULL,
#       Segundo vetor de dados, opcional. Se fornecido, realiza o teste de
#       Wilcoxon de duas amostras. Se NULL (padrão), realiza o teste de uma amostra.
#
#   alternative = c("two.sided", "less", "greater"),
#       Define a hipótese alternativa:
#       "two.sided" — a mediana difere do valor de referência (para mais ou para menos).
#       "less"      — a mediana é menor que o valor de referência.
#       "greater"   — a mediana é maior que o valor de referência.
#
#   mu = 0,
#       Valor numérico que indica o valor verdadeiro do parâmetro de locação
#       sob H₀ (i.e., o valor de referência para o teste de uma amostra).
#
#   paired = FALSE,
#       Lógico. Se TRUE, realiza o teste de Wilcoxon pareado (assinado).
#       Se FALSE (padrão), realiza o teste de postos de Wilcoxon (não pareado).
#
#   exact = NULL,
#       Lógico. Se TRUE, calcula o p-valor exato usando a distribuição exata
#       dos postos. Se FALSE, usa a aproximação normal. NULL (padrão) escolhe
#       automaticamente: exato se n <= 50 e sem empates, caso contrário aproximação.
#
#   correct = TRUE,
#       Lógico. Se TRUE, aplica a correção de continuidade na aproximação normal.
#       Só tem efeito quando exact = FALSE.
#
#   conf.int = FALSE,
#       Lógico. Se TRUE, retorna o intervalo de confiança e a estimativa pontual
#       da diferença de locação.
#
#   conf.level = 0.95
#       Nível de confiança para o intervalo de confiança (padrão: 0.95).
# )
```
...
2. mu: O respectivo valor de referência clínica universal  
3. alternative: "two.sided", pois queremos detectar diferença para mais ou para menos  
4. exact: FALSE, pois usamos a aproximação normal (nossa amostra é grande)  
5. correct: TRUE (correção de continuidade aplicada)  
