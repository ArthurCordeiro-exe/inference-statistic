...
skew_r <- function(x) {
  ...
  m <- mean(x, na.rm = TRUE)
  s <- sd(x, na.rm = TRUE)
  (n / ((n - 1) * (n - 2))) * sum(((x - m) / s)^3, na.rm = TRUE)
}

skewness_labels <- setNames(
  sapply(num_cols, function(col) round(skew_r(df[[col]]), 4)),
  num_cols
)

df_qq <- df_long |>
  mutate(
    assimetria = skewness_labels[Variavel],
    label      = paste0(Variavel, "\nAssimetria: ", assimetria)
  )

ggplot(df_qq, aes(sample = Valor)) +
  stat_qq() +
  stat_qq_line(color = "red") +
  facet_wrap(~label, scales = "free", ncol = 3) +
  labs(
    title = "Q-Q Plots das variáveis (grupo saudável)",
    x     = "Quantis Teóricos",
...
```{r}
df <- df |> select(-Insulin)
```

```{r}
resultados_shapiro <- map_dfr(num_cols <- names(df), function(col) {
  dados <- df[[col]]
